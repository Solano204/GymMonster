...
class PerTrainerInformationClientTest {
    ...
    private PerTrainerInformationClient repository;
    ...
    void setUp() {
        ...
        repository = new PerTrainerInformationClient(webClientBuilder, servicesUrl);
    ...
    @Test
    void createPerTrainer_postsToFixedEndpoint_withBody() {
        ...
        StepVerifier.create(repository.createPerTrainer(trainer)).expectNext("Created").verifyComplete();

        ArgumentCaptor<String> uriCaptor = ArgumentCaptor.forClass(String.class);
        verify(requestBodyUriSpec).uri(uriCaptor.capture());
        assertThat(uriCaptor.getValue()).isEqualTo(BASE_URL + "/api/pertrainers/AD/create");
    }

    @Test
    void getAllTrainers_buildsUriWithPageAndSizeQueryParams_asAFlux() {
    ...
    @Test
    void deletePerTrainer_deletesWithUsernameAndPasswordAsPathVariables() {
        ...
        StepVerifier.create(repository.deletePerTrainer("coach99", "pw")).expectNext("Deleted").verifyComplete();
    }

    @Test
    void getAllClientsFromTrainer_buildsUriWithUsernamePageAndSize_asAFlux() {
        when(webClient.get()).thenReturn(requestHeadersUriSpec);
        when(requestHeadersUriSpec.uri(any(java.util.function.Function.class))).thenReturn(requestHeadersSpec);
        when(requestHeadersSpec.accept(MediaType.APPLICATION_JSON)).thenReturn(requestHeadersSpec);
        when(requestHeadersSpec.retrieve()).thenReturn(responseSpec);
        ...
        StepVerifier.create(repository.getAllClientsFromTrainer("coach99", 0, 10))
                .expectNext(client)
                .verifyComplete();
    }
    ...
    @Test
    void updateTrainerPassword_putsWithUsernameNewPasswordAndOldPasswordAsPathVariables() {
        when(webClient.put()).thenReturn(requestBodyUriSpec);
        when(requestBodyUriSpec.uri(anyString(), eq("coach99"), eq("new-pw"), eq("old-pw"))).thenReturn(requestBodySpec);
        when(requestBodySpec.retrieve()).thenReturn(responseSpec);
        when(responseSpec.bodyToMono(String.class)).thenReturn(Mono.just("Password updated"));

        StepVerifier.create(repository.updateTrainerPassword("coach99", "new-pw", "old-pw"))
    ...
    @Test
    void updateTrainerDetails_putsWithUsernamePathVariable_andBody() {
        when(webClient.put()).thenReturn(requestBodyUriSpec);
        ...
        StepVerifier.create(repository.updateTrainerDetails("coach99", details))
                .expectNext("Updated")
                .verifyComplete();
    }
    ...
    @Test
    void removeSpecialtyFromTrainer_deletesWithUsernameAndSpecialtyAsPathVariables() {
        when(webClient.delete()).thenReturn(requestHeadersUriSpec);
        ...
        StepVerifier.create(repository.removeSpecialtyFromTrainer("coach99", "Yoga"))
