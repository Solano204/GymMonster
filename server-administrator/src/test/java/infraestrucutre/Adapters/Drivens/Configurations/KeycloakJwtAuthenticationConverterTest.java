...
class KeycloakJwtAuthenticationConverterTest {
    ...
    void convert_mapsKeycloakClientRolesToRoleAuthorities() {
        when(jwt.getClaim("resource_access")).thenReturn(
                Map.of("Docker-Gym", Map.of("roles", List.of("ADMIN", "STAFF"))));
        ...
        StepVerifier.create(converter.convert(jwt))
                .assertNext(token -> {
                    ...
                    List<String> authorities = token.getAuthorities().stream()
                            ...
                            .toList();
                    assertThat(authorities).contains("ROLE_ADMIN", "ROLE_STAFF");
                })
                .verifyComplete();
    }
    ...
    void convert_returnsNoResourceRoleAuthorities_whenOurClientIdIsAbsentFromResourceAccess() {
        when(jwt.getClaim("resource_access")).thenReturn(Map.of("some-other-client", Map.of("roles", List.of("ADMIN"))));
        when(jwt.getClaim("preferred_username")).thenReturn("jdoe");
    ...
    @Test
    void convert_returnsNoResourceRoleAuthorities_whenRolesListIsMissingFromOurClientEntry() {
        when(jwt.getClaim("resource_access")).thenReturn(Map.of("Docker-Gym", Map.of()));
        when(jwt.getClaim("preferred_username")).thenReturn("jdoe");
    ...
    @Test
    void convert_usesPrincipleAttributeClaim_asTheTokenName() {
        when(jwt.getClaim("resource_access")).thenReturn(null);
        when(jwt.getClaim("preferred_username")).thenReturn("carlos.custom.name");

        StepVerifier.create(converter.convert(jwt))
                .assertNext(token -> assertThat(token.getName()).isEqualTo("carlos.custom.name"))
                .verifyComplete();
    }
}
